...
export async function createGeorgianRecipeWithDetails(
  ...
  try {
    ...
    if (ingredients.length === 0) {
      console.warn('WARNING: No ingredients provided for recipe!');
    }
...
export async function createNews(newsData: typeof news.$inferInsert) {
  try {
    ...
    if (!newsData.title) {
      console.warn('WARNING: News article is missing title!');
    }
    
    if (!newsData.content) {
      console.warn('WARNING: News article is missing description!');
    }
